...
// --- 設定 ---
const inputMsgpackPath = path.join(__dirname, '../merge/T1.msgpack');
const outputJsonPath = path.join(__dirname, './T1.json'); // null にするとコンソール出力のみ
const jsonIndent = 2;
// --- 設定ここまで ---
    ...
    if (typeof value === 'string') {
        try {
            ...
            const cleanedString = value.replace(/[\u0000-\u001F\u007F-\u009F]/g, (char) => {
                ...
                console.warn(`[WARNING] Found control character in string value for key "${key}": \\u${char.charCodeAt(0).toString(16).padStart(4, '0')}. Replacing with '?'.`);
                return '?';
            });
            return cleanedString;

        } catch (e) {
            console.error(`[ERROR] Failed to process string for key "${key}": ${e.message}. Replacing with "[STRING_ERROR]". Original value start: "${value.substring(0, 50)}..."`);
...
async function convertMsgpackToJson(inputPath, outputPath = null, indent = null) {
    ...
    try {
        ...
        console.error(`This error likely means a non-serializable type or bad characters are present after MsgPack decoding.`);
...
convertMsgpackToJson(inputMsgpackPath, outputJsonPath, jsonIndent)
    .catch(error => {
