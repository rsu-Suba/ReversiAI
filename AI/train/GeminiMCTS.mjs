...
export class MCTS {
   constructor(rng = Math.random) {
      ...
      this.shouldStopSimulations = false; // ★★★ 新しいフラグを追加 ★★★
   }
   ...
   run(currentBoardState, currentPlayer, numSimulations) {
      this.shouldStopSimulations = false; // runが呼ばれるたびにリセット
      ...
      for (let i = 0; i < numSimulations; i++) {
         if (this.shouldStopSimulations) {
            console.log("MCTS.run: Stopping simulations early due to termination request.");
            break; // ループを抜ける
         }
